...
export interface MotorApplication {
  ...
  submittedAt: string;
  status: "Pending" | "Accepted" | "Rejected" | "Edited" | "AwaitingPayment";
  notes?: string;
  // Add other motor-specific fields as needed
}
...
// ============================================
// MOTOR APPLICATIONS (Pending Review)
// ============================================

/**
 * Get all motor applications pending FO approval
 */
export const getPendingMotorApplications = async (): Promise<MotorApplication[]> => {
  try {
    const response = await axios.get(`${BASE_URL}/motor/pending`);
    return response.data;
  } catch (error: any) {
    console.error("Failed to fetch pending motor applications:", error);
    if (error.response) {
...
export const getApprovedMotorApplications = async (): Promise<MotorApplication[]> => {
  try {
    const response = await axios.get(`${BASE_URL}/motor/approved`);
    return response.data;
  } catch (error: any) {
    console.error("Failed to fetch approved motor applications:", error);
    if (error.response) {
      console.error("Error response:", error.response.data);
      throw new Error(error.response.data.message || "Failed to fetch approved applications");
    }
    throw error;
  }
};
...
export const getRejectedMotorApplications = async (): Promise<MotorApplication[]> => {
  try {
    const response = await axios.get(`${BASE_URL}/motor/rejected`);
    return response.data;
  } catch (error: any) {
    console.error("Failed to fetch rejected motor applications:", error);
    if (error.response) {
      console.error("Error response:", error.response.data);
      throw new Error(error.response.data.message || "Failed to fetch rejected applications");
    }
    throw error;
  }
};
...
export const approveMotorApplication = async (id: string): Promise<void> => {
  try {
    await axios.post(`${BASE_URL}/motor/${id}/approve`);
  } catch (error: any) {
    console.error(`Failed to approve motor application ${id}:`, error);
    if (error.response) {
      console.error("Error response:", error.response.data);
 ...
 */
export const rejectMotorApplication = async (id: string, reason: string): Promise<void> => {
  try {
    await axios.post(`${BASE_URL}/motor/${id}/reject`, {
      message: reason
    });
  } catch (error: any) {
    console.error(`Failed to reject motor application ${id}:`, error);
    if (error.response) {
      console.error("Error response:", error.response.data);
...
/**
 * Update motor application (FO can edit premium, coverage, notes)
 */
export const updateMotorApplication = async (
  id: string, 
  updates: { 
    premium?: number; 
    coverageAmount?: number; 
  ...
  try {
    await axios.put(`${BASE_URL}/motor/${id}`, updates);
  } catch (error: any) {
    console.error(`Failed to update motor application ${id}:`, error);
    if (error.response) {
      console.error("Error response:", error.response.data);
...
/**
 * Get finance analytics/summary
 * Note: You might need to create this endpoint or calculate from existing data
 */
export const getFinanceAnalytics = async (): Promise<FinanceAnalytics> => {
  try {
    // If you have a dedicated analytics endpoint:
    // const response = await axios.get(`${BASE_URL}/analytics`);
    // return response.data;
    
    // Otherwise, calculate from existing endpoints:
    ...
    const totalRevenueImpact = approved.reduce((sum, app) => sum + (app.premium || 0), 0);
...
// ============================================
// CLIENT APPLICATIONS (Legacy - keep for compatibility)
// ============================================

/**
 * Get all applications (maps to motor applications for now)
 */
export const getApplications = async (): Promise<MotorApplication[]> => {
  return getPendingMotorApplications();
};
...
export const updateApplicationStatus = async (
  id: string, 
  status: "Accepted" | "Rejected" | "AwaitingPayment", 
  notes?: string
): Promise<void> => {
  if (status === "Accepted") {
    await approveMotorApplication(id);
  } else if (status === "Rejected") {
    await rejectMotorApplication(id, notes || "No reason provided");
  }
  // Handle AwaitingPayment status if needed
};
...
export const editApplication = async (
  id: string, 
  updatedFields: { premium?: number; coverageAmount?: number; notes?: string }
): Promise<void> => {
  await updateMotorApplication(id, updatedFields);
};
