    ...
    <ReusableTable
      ...
      renderRow={(user, index) => (
        ...
        >
          ...
          {/* Contact Info + key client fields */}
          <TableCell className="py-4 px-6">
            <div className="text-sm text-amber-900">{user.email || "—"}</div>
            <div className="text-sm text-amber-800">{user.mobilePhone || "—"}</div>
            {user.gender && (
              <div className="text-xs text-amber-700">Gender: {user.gender}</div>
            )}
            {(user.city || user.region) && (
              <div className="text-xs text-amber-700">{[user.city, user.subCity, user.region].filter(Boolean).join(", ")}</div>
            )}
            {user.nationalIdOrPassport && (
              <div className="text-xs text-amber-700">ID: {user.nationalIdOrPassport}</div>
            )}
            {user.createdAt && (
              <div className="text-xs text-amber-700">Joined: {new Date(user.createdAt).toLocaleDateString()}</div>
            )}
          </TableCell>
