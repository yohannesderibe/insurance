...
const CreateOperatorModal: React.FC<Props> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.dateOfBirth) newErrors.dateOfBirth = "Date of birth is required";
     if (!formData.city.trim()) newErrors.city = "City is required"; // ADD THIS LINE
    setErrors(newErrors);
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-amber-200">
        <div className="bg-gradient-to-r from-amber-600 to-amber-700 text-white px-6 py-4 flex justify-between items-center">
          <h2 className="text-lg font-semibold">Create Operation Officer</h2>
          <button 
            onClick={onClose} 
                  ...
                  {imageFile ? (
                    ...
                    <div className="text-center">
                      <input
                        ...
                        className="hidden"
                        id="operatorImageUpload"
                        disabled={loading}
                      />
                      <label
                        htmlFor="operatorImageUpload"
      ...
      {subCities.map((subCity) => (
        <option key={subCity} value={subCity}>
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-amber-100">
            ...
            >
              {loading ? "Creating..." : "Create Operation Officer"}
            </button>
          </div>
        </div>
      </div>
