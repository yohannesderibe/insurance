...
const DetailsModal: React.FC<{
  ...
  onReject: () => void;
  onInReview: () => void;
}> = ({ claim, onClose, onApprove, onReject, onInReview }) => {
  if (!claim) return null;
...
<<<<<<< HEAD
          ...
          {claim.status === 'SentBack' && (
            <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-4 flex gap-3">
              ...
              <div>
                <h4 className="font-bold text-red-800">Returned by Manager</h4>
                <p className="text-red-700 text-sm mt-1">Manager Feedback: {claim.managerFeedback || "No feedback provided."}</p>
                <p className="text-red-600 text-xs mt-2">Please review the feedback and resubmit (Approve/Reject) appropriately.</p>
              </div>
            </div>
          )}
...
<<<<<<< HEAD
            ...
            {canAct && (
              <div className="flex gap-3">
                <button
                  onClick={onInReview}
                  className="px-5 py-2.5 rounded-lg bg-yellow-400 text-yellow-900 hover:bg-yellow-500 transition-colors flex items-center gap-2"
                >
                  <FileText className="w-5 h-5" />
                  Mark In Review
                </button>
                  ...
                  return (
                    <TableRow key={row.id} className="hover:bg-amber-50">
                      ...
                      <TableCell className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${row.type === "Motor" ? "bg-blue-100 text-blue-800" : "bg-purple-100 text-purple-800"
                          }`}>
                          {row.type}
                        </span>
                      </TableCell>
                      ...
                      <TableCell className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${row.status.includes('Approved') ? 'bg-green-100 text-green-800' :
                            row.status.includes('Rejected') ? 'bg-red-100 text-red-800' :
                              row.status === 'Submitted' || row.status === 'Pending' ? 'bg-yellow-100 text-yellow-800' :
                                row.status === 'SentBack' ? 'bg-red-100 text-red-800' :
                                  'bg-blue-100 text-blue-800'
                          }`}>
                          {row.status}
                        </span>
