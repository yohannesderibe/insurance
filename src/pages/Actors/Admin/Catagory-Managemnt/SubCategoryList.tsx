...
const SubCategoryList: React.FC<Props> = ({ onEdit, onCreate, onView, refreshKey = 0 }) => {
  ...
  const filtered = subCategories.filter((s) =>
    `${s.name} ${s.description} ${getCategoryName(s.parentId)}`
      .toLowerCase()
      .includes(searchTerm.toLowerCase())
  );
    ...
    <>
      ...
      <div className="mb-6 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <AddButton 
            ...
            title={!selectedParentId ? "Please select a parent category first" : "Add Subcategory"}
          />
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
          <div className="flex-1">
            ...
            <select
              ...
              className="w-full max-w-xs border border-amber-200 rounded-lg p-2 text-sm focus:ring-1 focus:ring-amber-500"
            >
            ...
            <p className="text-xs text-amber-600 mt-1">
              Select a parent category to view and manage its subcategories
            </p>
          </div>
        </div>
      </div>

      {/* Search / Filter - Only show if parent category is selected */}
      {selectedParentId && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-amber-200 mb-6">
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
            <div className="flex-1 w-full relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-amber-500" />
              <SearchBar
                placeholder="Search subcategory by name, description..."
                value={searchTerm}
                onChange={(v) => {
                  setSearchTerm(v);
                  setCurrentPage(1);
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-amber-200 overflow-hidden">
        {!selectedParentId ? (
          <div className="text-center py-12">
            <div className="text-amber-400 text-6xl mb-4">📂</div>
            <h3 className="text-lg font-semibold text-amber-800 mb-2">Select a Parent Category</h3>
            <p className="text-amber-600">
              Please select a parent category from the dropdown above to view its subcategories
            </p>
          </div>
        ) : loading ? (
          <div className="flex justify-center items-center py-12">
...
<TableCell className="py-3 px-4 text-center text-amber-700">
  {row.thirdPartyPercentage?.toFixed(2) || "N/A"}%
        ...
        {/* Empty state */}
        {selectedParentId && !loading && filtered.length === 0 && (
          <div className="text-center py-12">
            <div className="text-amber-400 text-6xl mb-4">📂</div>
            <h3 className="text-lg font-semibold text-amber-800 mb-2">No subcategories found</h3>
            <p className="text-amber-600">
